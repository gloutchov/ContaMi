...
- Il modulo Automobile può creare e modificare un finanziamento facoltativo insieme al mezzo: esiste una sola Ricorrenza collegata, ogni scadenza genera una registrazione `Pagamento rateale`, chiusura/riapertura restano sincronizzate e lo storico confermato non viene cancellato. Da **Nuova spesa / rilevazione**, ogni costo non di valutazione può essere diviso automaticamente a metà come Spesa condivisa. Inserendo i chilometri totali viene proposta la distanza dall’ultima lettura certa della stessa automobile; nei rifornimenti importo e prezzo al litro calcolano i litri e l’alimentazione si sceglie da un elenco bilingue facoltativo.
- Grafici per ogni investimento e comparto pensione che confrontano nel tempo capitale investito e controvalore e, subito sotto, mostrano il rendimento mensile Modified Dietz fra valutazioni consecutive, depurato dai flussi. Il primo Versamento costituisce capitale iniziale, le lacune restano senza punto ma non interrompono la linea, il tooltip è sopra l’area tracciata e una linea tratteggiata indica la media aritmetica visibile. Nell’anno di apertura la card privilegia l’andamento mensile; dagli anni successivi passa a quello annuale, con stime ricavate da consuntivi di chiusura consecutivi e periodi parziali dichiarati. La pagina Investimenti confronta inoltre il rendimento annuo globale di Titoli, Fondi e Fogli, aggregando valori e flussi di ciascuna tipologia prima del calcolo e non mediando le percentuali individuali. Pagina, schede e dettagli mostrano anche capitale iniziale, versamenti successivi, liquidazioni totali e saldo investito/liquidato; **Correzione** rettifica solo questi riepiloghi e non entra nel rendimento. Nuovo/Modifica investimento, i comparti pensione e Correzione accettano inoltre un ISIN facoltativo, validato localmente e salvato in maiuscolo sulla posizione.
...
- The Vehicles form can create and edit optional financing together with the vehicle: one linked Recurring Item owns the plan, each occurrence creates an Installment vehicle record, close/reopen stays synchronized, and confirmed history cannot be deleted. From **New cost / reading**, every non-valuation cost can be automatically split in half as a Shared Expense. Entering the odometer suggests the distance from the latest certain reading for the same vehicle; for fuel records, amount and unit price calculate litres and fuel type comes from an optional bilingual list.
- Per-position investment and pension-compartment charts compare net invested capital with countervalue and then show cash-flow-adjusted monthly Modified Dietz returns between consecutive valuations. The first Contribution is opening capital, gaps keep no point but do not break the line, the tooltip sits above the plot, and a dashed line marks the visible arithmetic mean. During the opening year, each card favours the monthly trend; from later years it switches to the annual trend, with estimates derived from consecutive closing summaries and partial periods identified. The Investments page also compares the global annual return of Securities, Funds, and Savings plans by aggregating each type’s values and flows before calculation, never by averaging individual percentages. Pages, cards, and details also show initial invested capital, subsequent contributions, total liquidations, and invested/liquidated balance; **Correction** changes those summaries only and is excluded from returns. New/Edit Investment, pension compartments, and Correction also accept an optional ISIN, validated locally and stored in uppercase on the position.
